/**
 * Draw Mermaid - loads SVG assets and renders them on canvas
 * Underwater fairy tale style characters
 */
...
const ASSET_LIST = {
  mom: 'assets/mermaid-mom.svg',
  baby: 'assets/baby-mermaid.svg',
  babyHappy: 'assets/baby-mermaid-happy.svg',
  babySad: 'assets/baby-mermaid-sad.svg',
  babyAngry: 'assets/baby-mermaid-angry.svg',
};
...
// Preload all assets
export async function preloadMermaidAssets() {
...
const _preloadPromise = preloadMermaidAssets();
export { _preloadPromise as mermaidAssetsReady };
...
// Draw baby Mermaid using SVG assets
export function drawBabyMermaid(ctx, x, y, size, emotion, phase, growthRatio) {
  ...
  // Tiny bubble near baby when bigger
  if (growthRatio > 0.6) {
    const sparkAlpha = 0.3 + Math.sin(phase * 3) * 0.2;
    ctx.globalAlpha = sparkAlpha;
    ctx.font = `${size * 0.3}px sans-serif`;
